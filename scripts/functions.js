...
class Busca {
  ...
  /*custoUniforme(inicio, objetivo){
        let visitado = [];
        let linha = [];
        this.caminho.clear();
        let lista = listaDpEncadeada();
        let listaC = listaDpEncadeada();
        lista.adicionaFim(inicio, null, 0);
        listaC.adicionaFim(inicio, null, 0);
        linha.push(inicio);
        linha.push(0);
        visitado.push(linha);
        while (lista.verifica() != null){
            atual = lista.deletaInicio();
            if (objetivo.atual.carga){
                custo = atual.nivel;
                cam = listaC.exibeArvore(atual.carga, atual.nivel);
                this.caminho.push(custo);
                this.caminho.push(cam.reverse());
                objetivo.remove(atual.carga);
            }
            if (objetivo.length() == 0){
                return this.caminho;
            }else{
                ind = nos.index(atual.carga);
                valor = atual.nivel;
                for (let i= 0; i < grafoPon.length; i++){
                    novo = grafoPon[ind][i][0];
                    bandeira = True;
                    novocusto = valor + grafoPon[ind][i][1];
                    for (let j = 0; j < visitado.length; j++){
                        if (visitado[j][0] == novo){
                            if (visitado[j][1] <= novocusto){
                                bandeira = False
                            break
                            }
                        }
                    ...
                    if (bandeira){
                        lista.adicionaNaLista(novo, atual, novocusto);
                        listaC.adicionaFim(novo, atual, novocusto);
                        linha = [];
                        linha.push(novo);
                        linha.push(novocusto);
                        visitado.push(linha);
                    }
                }
            }
...
class ListaDpEncadeada {
    ...
    adicionaPrimeiro(c, p, n){
        let novoNode = new Node(c, p, n);
        ...
        if(this.head == null) {
            ...
            novoNode.proximo2 = this.head;
            this.head.anterior = novoNode;
            this.head = novoNode;
        }
    }
    ...
    adicionaFim(c, p, n){
        ...
        if(this.head == null) {
            ...
            novoNode.proximo2 = novoNode;
            novoNode.anterior2 = this.tail;
            this.tail = novoNode;
        }
    }

    adicionaNaLista(novo, p, novocusto){
        let novoNode = new Node(novo, p, novocusto);
